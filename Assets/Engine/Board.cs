...
namespace Mancala
{
    ...
    public class Board
    {
        ...
        public int makeMove(int move, bool chatter)
        {
            ...
            {
               		 if (board[7] > 0 && board[9] == 0 && board[8] == 0 && board[10] == 0 && board[11] == 0 && board[12] == 0)
                    {
				...
				if (chatter) UnityEngine.Debug.Log("TOP captured {0} stones!" + capture);
...
//				if (chatter) UnityEngine.Debug.Log("TOP captured {0} stones!", capture);
            ...
            if (playerToMove == Position.Bottom && pos >= 0 && pos < 6 && board[pos] == 1 && board[12 - pos] > 0)
            {
				...
				if (chatter) UnityEngine.Debug.Log("BOTTOM captured {0} stones!" + capture);
            ...
            if (playerToMove == Position.Top)
            {
                if (pos != 13)
					...
					if (chatter) UnityEngine.Debug.Log("TOP goes again.");
            }
            else
                if (pos != 6)
                playerToMove = Position.Top;
            else
					if (chatter) UnityEngine.Debug.Log("BOTTOM goes again.");
        ...
        public void display()
        {
			...
			for (int i = 0; i < board.Length; i++) {
				str += "[" + i.ToString() + "-" + board [i].ToString () + "]";
			}
